...
for speaker in "${bases_very_low[@]}"; do
    python asr_transf.py "$speaker" original True False True 100 encoder_sgd_epoch100.h5
done
...
for speaker in "${bases_mild[@]}"; do
    python asr_transf.py "$speaker" original False True True 100 encoder_sgd_epoch100.h5
done
