...
import argparse  # For distributed arguments

# Definir `local_rank` e `rank` corretamente
rank = int(os.environ.get("RANK", 0))
local_rank = int(os.environ.get("LOCAL_RANK", 0))
num_gpus = torch.cuda.device_count()

# Ajustar `local_rank` para garantir que ele está dentro do número de GPUs disponíveis
if local_rank >= num_gpus:
    print(f"⚠️ WARNING: LOCAL_RANK={local_rank}, mas há apenas {num_gpus} GPU(s). Usando GPU 0.")
    local_rank = 0  # Fallback para a única GPU disponível

# Definir `device` corretamente
torch.cuda.set_device(local_rank)
device = torch.device(f"cuda:{local_rank}" if torch.cuda.is_available() else "cpu")

print(f"✅ Using device: {device} (RANK={rank}, LOCAL_RANK={local_rank})")

# ⚠️ Agora garantimos que `device` é válido antes de inicializar o modelo
if device.index is not None and device.index >= num_gpus:
    print(f"⚠️ WARNING: Tentando usar cuda:{device.index}, mas apenas {num_gpus} GPU(s) estão disponíveis.")
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

print(f"✅ Dispositivo final ajustado para: {device}")


def setup_distributed_training(master_ip, port, rank, world_size):
    dist.init_process_group(
        backend="nccl",  # For GPU support
        init_method=f"tcp://{master_ip}:{port}",
        rank=rank,
        world_size=world_size
    )
    torch.cuda.set_device(rank)
    print(f"Rank {rank} initialized on {master_ip}:{port}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--master_ip", type=str, required=True, help="IP address of the master node")
    ...
    setup_distributed_training(args.master_ip, args.port, args.rank, args.world_size)
...
def create_dataloaders_distributed(base_path, batch_size, collate_fn, rank, world_size):
    print("Creating Distributed DataLoaders...")
    datasets = {
        "train": UASpeechDataset(base_path, "train", preprocessor),
        "dev": UASpeechDataset(base_path, "dev", preprocessor),
        "test": UASpeechDataset(base_path, "test", preprocessor),
    }
...
dysarthric_dataloaders = create_dataloaders_distributed(
    dysarthric_base_path, batch_size, collate_fn, args.rank, args.world_size
)
control_dataloaders = create_dataloaders_distributed(
    control_base_path, batch_size, collate_fn, args.rank, args.world_size
)
...
def apply_time_stretch(audio: torch.Tensor, sample_rate: int, factor: float = 0.9) -> torch.Tensor:
    ...
    resampler = torchaudio.transforms.Resample(orig_freq=sample_rate, new_freq=new_sample_rate).to(device='cuda:0')
...
#Dimesity reduction
def extract_embeddings(dataloader, model, device, fraction=0.01, max_samples=1000):
    """Extrai embeddings sem converter todo o dataloader para lista, evitando estouro de memória."""
    model.eval()
    embeddings = []
    indices = []
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            if random.random() > fraction:  # 🔥 Pula batches aleatoriamente para manter 1% do dataset
                continue
            
            inputs = batch[0].to(device, non_blocking=True)  # Move para GPU
            output = model(inputs)  # Extrai embeddings
            
            min_length = min(min_length, output.size(1))  
            embeddings.append(output.cpu())  # 🔥 Move para CPU imediatamente para liberar memória
            indices += [i] * inputs.size(0)

            torch.cuda.empty_cache()  # 🔥 Libera memória da GPU

    if len(embeddings) == 0:
        raise RuntimeError("Nenhum embedding foi extraído. Reduza 'fraction' para incluir mais amostras.")

    # Truncar embeddings para o menor tamanho encontrado
    truncated_embeddings = [emb[:, :min_length] for emb in embeddings]
    embeddings = torch.cat(truncated_embeddings, dim=0)  
    indices = torch.tensor(indices)

    # Amostragem para visualização
    if embeddings.shape[0] > max_samples:
        sampled_indices = torch.randperm(embeddings.shape[0])[:max_samples]
...
def train_simclr(model, dataloader, optimizer, criterion, device, epochs):
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs} Training Batches", leave=True), 1):
            #print(f"🔍 RANK={rank}: Processando batch com {len(batch)} amostras", flush=True)
            x, _ = batch
        ...
        if (epoch + 1) % freq_saveup == 0:
            checkpoint_filename = f"{checkpoint_path}/simclr_epoch_{epoch+1}.pt"
            torch.save(model.state_dict(), checkpoint_filename)
            print(f"Checkpoint saved: {checkpoint_filename}")
        
        # Extract e plot embeddings each 10 epochs
        if (epoch + 1) % freq_tsne == 0:
            embeddings, indices = extract_embeddings(control_dataloaders['dev'], simclr_model, device='cuda:0', fraction=0.1, max_samples=128)
...
def plot_curves(loss_train_list, loss_val_list, train_acc_list, val_acc_list, loss_or_acc):
    ...
    sigma = 3  # Controle da suavização
    train = gaussian_filter1d(train, sigma) 
    val = gaussian_filter1d(val, sigma)
...
def load_model_checkpoint(model, checkpoint_path, device):
    ...
    if os.path.exists(checkpoint_path):
        ...
        if 'model_state_dict' in checkpoint:
            ...
            print(f"Loaded model parameters directly from {checkpoint_path}")
        else:
            print(f"Checkpoint format not recognized: {checkpoint_path}. Training from scratch.")
    else:
        print(f"Checkpoint {checkpoint_path} not found. Training from scratch.")
    return model

checkpoint_path = "models/ds_wav2vec_sch-tmax10_lr-1e-4_ep-100.pth"

# Section 9: Training loop for downstream (ASR)
def train_downstream(model, dataloader, optimizer, scheduler, vocab, device):
    """Train the downstream model with ASR objective."""
    ...
    for batch in progress_bar:
        ...
        with torch.no_grad():
            ...
            for i in range(batch_size):
                ...
                if target_length > 0:
                    pred_seq = preds[i][:target_length].cpu().numpy()
                    target_seq = targets[start_idx:start_idx + target_length].cpu().numpy()
                    start_idx += target_length
                    
                    # Remover tokens de blank (0) das predições
                    ...
                    if min_length > 0:
                        correct_preds = (pred_seq[:min_length] == target_seq[:min_length]).sum()
                        total_batch_correct += correct_preds
                        total_batch_samples += min_length
            
            total_correct += total_batch_correct
            total_samples += total_batch_samples
        
        acc = total_correct / total_samples if total_samples > 0 else 0
        '''
        progress_bar.set_postfix(loss=loss.item(), LR=(f"{current_lrs:.2e}"))
...
def validate_downstream(model, dataloader, vocab, device):
    ...
    with torch.no_grad():
        ...
        word_acc = 1 - val_wer
        char_acc = 1 - val_cer
        print(f"Average WER: {val_wer:.4f}, Average Word Accuracy: {word_acc:.4f}")
        print(f"Average CER: {val_cer:.4f}, Average Character Accuracy: {char_acc:.4f}")
        
        val_loss_list.append(val_loss)
        val_cer_list.append(val_cer)
        val_wer_list.append(val_wer)
        val_acc_list.append(word_acc)
...
simclr_model = DDP(simclr_model, device_ids=[args.rank])

print(simclr_model)


#Started Embeddings for compare progress
...
for epoch in range(us_epochs):
    ...
    train_loss = train_simclr(simclr_model, control_dataloaders["train"], optimizer, criterion, device='cuda:0', epochs=us_epochs)
...
for epoch in range(ds_epochs):
    ...
    save_dir = 'models'
    save_filename = 'ds_wav2vec_sch-tmax10_lr-1e-4_ep-100.pth'
